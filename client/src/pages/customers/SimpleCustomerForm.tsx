...
export default function SimpleCustomerForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const customerCode = `KH${(customerCount + 1).toString().padStart(4, '0')}`;
      const vehicleCode = `XE${(vehicleCount + 1).toString().padStart(4, '0')}`;

      // Create clean customer object
      const cleanCustomer = {
        code: customerCode,
      ...
      const cleanVehicle = {
        ...
        lastOdometer: vehicleData.lastOdometer ? Number(vehicleData.lastOdometer) : 0
      };

      // Remove undefined values
      if (cleanVehicle.year === undefined) {
        delete cleanVehicle.year;
      }
