...
// Định nghĩa màu cam trực tiếp - không cần biến trung gian vì có thể gây lỗi
    ...
    >
      <div className="flex items-center justify-between h-16 px-4 border-b border-sidebar-border">
        <div className="flex items-center space-x-2 overflow-hidden">
          <i className="fas fa-car-mechanic text-2xl text-orange-500"></i>
          <h1
        ...
        <div className="mb-1">
          <Link href="/invoices">
            <a className={isActive('/invoices') ? 'sidebar-link-active' : 'sidebar-link'}>
              <i className={`fas fa-receipt w-6 ${orangeColor}`}></i>
              <span className={`ml-2 truncate whitespace-nowrap ${!isExpanded && 'opacity-0'}`}>
                Hóa Đơn
              </span>
            </a>
        ...
        <div className="mb-1">
          <Link href="/reports">
            <a className={isActive('/reports') ? 'sidebar-link-active' : 'sidebar-link'}>
              <i className={`fas fa-chart-line w-6 ${orangeColor}`}></i>
              <span className={`ml-2 truncate whitespace-nowrap ${!isExpanded && 'opacity-0'}`}>
                Báo Cáo
              </span>
            </a>
      ...
      <div className="border-t border-sidebar-border p-4">
        <Link href="/settings">
          <a className="flex items-center text-gray-400 hover:text-white">
            <i className={`fas fa-cog w-6 ${orangeColor}`}></i>
            <span className={`ml-2 truncate whitespace-nowrap ${!isExpanded && 'opacity-0'}`}>
              Cài Đặt
            </span>
          </a>
