  ...
  if (isPrintMode) {
    return (
      <div id="invoice-print" className="bg-white p-8 max-w-4xl mx-auto font-sans text-sm">
        ...
        {/* Title */}
        <h2 className="text-2xl font-bold text-center mb-6">HÓA ĐƠN QUYẾT TOÁN</h2>
        ...
        <div className="mb-6 p-4 bg-green-50 border border-green-200">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p>Số tiền đã thanh toán: <strong>{formatCurrency(amountPaid)}</strong></p>
              <p>Phương thức: <strong>{getPaymentMethodText(paymentMethod)}</strong></p>
            </div>
            <div>
              <p>Số tiền còn lại: <strong className={remainingAmount > 0 ? 'text-red-600' : 'text-green-600'}>
                {formatCurrency(remainingAmount)}
              </strong></p>
              <p>Trạng thái: <strong className={remainingAmount === 0 ? 'text-green-600' : 'text-orange-600'}>
                {remainingAmount === 0 ? 'Đã thanh toán' : remainingAmount < total ? 'Thanh toán một phần' : 'Chưa thanh toán'}
              </strong></p>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-between mt-12">
          ...
          <div className="text-center w-1/3">
            <p className="font-bold mb-16">THU NGÂN</p>
            <p className="border-t border-black pt-2">(Ký và ghi rõ họ tên)</p>
          </div>
        </div>
